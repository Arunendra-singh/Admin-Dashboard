...
const AddRowToPage = ({ addNewRowToConfig }) => {
    ...
    return (
        <div id="add-row-to-page" className="skip_iframe">
            {rowDivider.map((buttonData, index) => (
                <Button ariaLabel={buttonData.label} id={`btnaddrow_${index + 1}`} key={buttonData.label} buttonType="button" label={buttonData.label} onClick={() => onSubmit(buttonData.colSpan)} />
            ))}
        </div>
    );
};
