...
import { AddIndustriesAPI } from "common/components/graphQL/queries/industries/industries";
import { PopupV3 } from "~/helpers/PopupV3";

const AddIndustries = ({ showPopup, closePopup }) => {
    const [AddEventThemeData] = useMutation(AddIndustriesAPI);
    ...
    const onSubmit = async (formData) => {
        AddEventThemeData({
            ...
            if (data.data.addUpdateIndustries.statuscode === 200) {
                closePopup();
                PopupV3({
                    content: "<p class=text-center>Industry has been added successfully.</p>",
                    type: "Success",
                ...
                PopupV3({
                    content: `<p class=text-center>${data.data.addUpdateIndustries.message}</p>`,
                    type: "Warning",
                    title: "Alert",
    ...
    return (
        <Modal className="EventThemePopup" size="md" show={showPopup} onHide={() => closePopup()}>
            <ModalHeader closeButton={true}>
                <ModalTitle>Add Industry</ModalTitle>
            </ModalHeader>
            <ModalBody>
