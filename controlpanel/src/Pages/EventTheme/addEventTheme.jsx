...
import { AddEventThemeAPI } from "common/components/graphQL/queries/eventsthemes/event-themes";
import { PopupV3 } from "~/helpers/PopupV3";

const AddEventTheme = ({ showPopup, closePopup }) => {
    const [AddEventThemeData] = useMutation(AddEventThemeAPI);
    ...
    const onSubmit = async (formData) => {
        AddEventThemeData({
            ...
            if (data.data.addUpdateEventThems.statuscode === 200) {
                closePopup();
                PopupV3({
                    content: "<p class=text-center>Event theme has been added successfully.</p>",
                    type: "Success",
                ...
                PopupV3({
                    content: `<p class=text-center>${data.data.addUpdateEventThems.message}</p>`,
                    type: "Warning",
                    title: "Alert",
    ...
    return (
        <Modal className="EventThemePopup" size="md" show={showPopup} onHide={() => closePopup()}>
            <ModalHeader closeButton={true}>
                <ModalTitle>Add Event theme</ModalTitle>
            </ModalHeader>
            <ModalBody>
                <form className="EditThemeform" onSubmit={handleSubmit(onSubmit)}>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            type="text"
                            {...register("Name", {
                                required: "Enter Valid Event Theme Name",
                                pattern: {
                                    value: /^(?! ).+/,
                                    message: "Please Enter Valid Event Theme Name"
                                }
                            })}
                        />
                    </div>
