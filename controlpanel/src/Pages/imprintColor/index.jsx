...
import { DeleteImprintColorAPI, useQueryGetAllImprintColors } from "common/components/graphQL/queries/ImprintColor/imprint-color";
...
import AddImprintColor from "./addImprintColor";
import EditImprintColor from "./editImprintColor";
...
const ImprintColorListing = () => {
    // sortby dropdown variables
    const [DeleteEventThemeData] = useMutation(DeleteImprintColorAPI);
    ...
    const {
        ...
        refetch,
        loading
    } = useQuery(useQueryGetAllImprintColors, {
        variables: {
            pageno: currentPage,
    ...
    const handleDropdownSort = (columnName) => {
        ...
        if (columnName.columnName === "Name A-Z") {
            setQueryEventOrder({ colorName: "ASC" });
        } else if (columnName.columnName === "Name Z-A") {
            setQueryEventOrder({ colorName: "DESC" });
        }
        setCurrentPage(1);
    ...
    const columns = ["Name", "Color Hex Value", "Action"];
    const columnAlignments = {
        Name: "clsImprintColor_Name text-left noTooltip",
        "Color Hex Value": "clsImprintColor_ColorHexValue text-left",
        Action: "clsImprintColor_Action text-no-ellipsis noTooltip"
    };
    useEffect(() => {
        if (eventData !== undefined) {
            setEventThemesData([]);
            setEventThemesDataFromAPI([]);
            eventData?.imprintColors?.items?.forEach((element) => {
                const objectArray = [
                    { rowValue: element.colorName, isLink: true, linkValue: "/v2/EventThemes=ABC" },
                    { rowValue: element.colorHexValue, isLink: true, linkValue: "/v2/EventThemes=ABC" },
                    [
                        { label: "Edit", icon: "icon icon-edit", id: element.imprintColorGuid },
                        { label: "Delete", icon: "icon icon-trash-2", id: element.imprintColorGuid }
                    ]
                ];
                setEventThemesData((prevItems) => [...prevItems, objectArray]);
            });
            const tPages = Math.ceil(eventData.imprintColors.totalCount / PagePerRecord);
            setTotalPages(tPages);
            setTotalRecords(eventData.imprintColors.totalCount);
            setEventThemesDataFromAPI(eventData?.imprintColors?.items);
        }
    }, [eventData]);
    ...
    const RemoveTheme = (Id) => {
        DeleteEventThemeData({
            ...
            if (data.data.deleteImprintColors.statuscode === 200) {
                setQueryEventSearch(null);
                setEventThemesData([]);
                setSearchValue("");
                refetch();
                PopupV3({
                    content: "<p class=text-center>Imprint color has been deleted successfully</p>",
                    type: "Success",
                ...
                PopupV3({
                    content: `<p class=text-center>${data.data.deleteImprintColors.message === "" ? "You can't delete this imprint color as this is already assigned to the products. please un-assign and try again." : data.data.deleteImprintColors.message}</p>`,
                    type: "Warning",
                    title: "Alert",
    ...
    useEffect(() => {
        if (searchValue === "") {
            ...
            setQueryEventSearch({ colorName: { contain: searchValue } });
        }
    }, [debouncedSearchTerm]);
            ...
            <PageContainer fluid classes="page-container eventthemes">
                <section className="search-bar mb-4">
                    <div className="search-bar-controls">
                        <div className="ShowResultDiv ml-3 mr-3 ml-lg-4 mr-lg-4">{eventData !== undefined && <ShowingResults pageRecords={PagePerRecord < totalRecord ? PagePerRecord : totalRecord} totalRecords={totalRecord} currentPage={currentPage} pageName="Imprint Color" />}</div>
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" fill="#405660" height="18px" width="18px" version="1.1" viewBox="0 0 383.748 383.748" xmlSpace="preserve"><g><path d="M62.772,95.042C90.904,54.899,137.496,30,187.343,30c83.743,0,151.874,68.13,151.874,151.874h30 C369.217,81.588,287.629,0,187.343,0c-35.038,0-69.061,9.989-98.391,28.888C70.368,40.862,54.245,56.032,41.221,73.593 L2.081,34.641v113.365h113.91L62.772,95.042z" /><path d="M381.667,235.742h-113.91l53.219,52.965c-28.132,40.142-74.724,65.042-124.571,65.042 c-83.744,0-151.874-68.13-151.874-151.874h-30c0,100.286,81.588,181.874,181.874,181.874c35.038,0,69.062-9.989,98.391-28.888 c18.584-11.975,34.707-27.145,47.731-44.706l39.139,38.952V235.742z" /></g></svg>
                            ...
                            <div className="input-wrapper searchField mr-2">
                                ...
                                <input type="search" aria-label="Search" onChange={searchOnChange} className="form-control clsImprintColor_Search" placeholder="Search..." value={searchValue} />

                                <button className="clear-button clsImprintColor_Reset" type="button" onClick={onSearchClose}>
                                    &times;
                                </button>
                            </div>
                            ...
                            <button className="btn btn_create clsImprintColor_Create" type="button" onClick={() => AddTheme()}>
                                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M5.25 6.65625H0.75V5.15625H5.25V0.65625H6.75V5.15625H11.25V6.65625H6.75V11.1562H5.25V6.65625Z" fill="white" />
                                </svg>
                                &nbsp; Create Imprint Color
                            </button>
                        </div>
                    </div>
                </section>
                <section className="listing-table mb-4 ml-3 mr-3 ml-lg-4 mr-lg-4">
                    <Datatable
                        ...
                        tourClass="clsImprintColor"
                        actionButtonStyle="inline" // You can change to "dropdown" for dropdown action buttons
                    />
                    {loading && <Loader />}
                </section>
                {eventData !== undefined && <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={handlePageChange} tourClass="clsImprintColor" />}
                {showEdit && <EditImprintColor showPopup={showEdit} closePopup={closePopup} Data={editThemeData} />}
                {showAdd && <AddImprintColor showPopup={showAdd} closePopup={closePopup} />}
            </PageContainer>
        </section>
    );
};

export default ImprintColorListing;
